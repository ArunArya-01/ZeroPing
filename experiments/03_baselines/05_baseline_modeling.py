...
Run:
    python experiments/03_baselines/05_baseline_modeling.py
...
def load_and_clean() -> tuple[pl.DataFrame, int, int]:
    if not PARQUET.exists():
        raise FileNotFoundError(f"{PARQUET} not found. Run the builder first.")

    df = pl.read_parquet(PARQUET)
    if "flight_id" not in df.columns:
        raise ValueError(
            "featured_dataset.parquet is missing flight_id. "
            "Run: python -m aerotwin.engine.build_featured_dataset --patch-flight-id"
        )
